...
function Hero({menu, isLogin, setIsLogin, userAuthPopUp}) {
  ...
  function handleMenu(id,  category) {
    setSelected(id)
    setSubMenu(true)
    let filteredData = items.filter((i) => (i.category === category))
  ...
  return (
    <div className='hero relative'>
      ...
      <div className=' bg-white w-[165px] h-[300px] absolute border-2'>
          <div className='p-2'>
            {items.map((item) => (
              <button onClick={()=> handleMenu(item.id, item.category)} key={item.id} className={`font-semibold text-[12px] 
      ...
      <div className='border-2 w-[300px] absolute right-0 bg-white'>
        ...
        <form className='mt-2'>
            <div className='flex flex-col w-[90%] m-[auto] gap-1 p-2 font-bold'>
              <label htmlFor="username" className='text-[13px]'>Email Address</label>
